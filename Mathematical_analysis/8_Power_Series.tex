...
&=\ssum{\int_0^x2\dfrac{(-1)^{(n-1)}}{(2n-1)}x^{2n-1}\diff x}\\
&=\int_0^x\lrp{\ssum{\int_0^x2(-1)^{n-1}x^{2n-2}\diff x}}\diff x\\
&=2\int_0^x\int_0^x\lrp{\ssum{(-x^2)^{n-1}}}\diff x\diff x\\
&=2\int_0^x\int_0^x\dfrac{1}{1+x^2}\diff x\diff x\\
&=2\int_0^x\arctan x\diff x\qquad\mbox{分部积分}\\
...
\end{enumerate}
\end{example}
\par 用\ref{sec:subsub:series_operation}节柯西乘积的方式，可求得更多级数的泰勒展开式.
\begin{example}
\[\begin{aligned}
(\arctan x)^2&=\lrp{\int_0^x\dfrac{1}{1+x^2}\diff x}^2\\
&=\lrp{\int_0^x\ssumz{(-1)^nx^{2n}}\diff x}^2\\
