...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{algorithm}[H]
            \caption{不精确线搜索}
            \begin{algorithmic}[1]
                \State{$\iter{\alpha}{k}=\alpha_{\text{max}}$}
...
若方程组非线性，那就做一个线性化
\begin{argmini*}
{\vd}{f_0(\iter{\vx}{k}+\vd)=\iter{\vd}{k}}{}{}
\addConstraint{A(\iter{\vx}{k}+\vd)}{=\vb}
\end{argmini*}
近似等价于（二阶近似Taylor展开）
\begin{argmini*}
{\vd}{f_0(\iter{\vx}{k})+\lrang{\nabla f_0(\iter{\vx}{k}),\vd}+\frac{1}{2}\vd^\T\nabla^\T f_0(\iter{\vx}{k})\vd=\iter{\vd}{k}}{}{}
\addConstraint{A(\iter{\vx}{k}+\vd)}{=\vb}
\end{argmini*}
...
即为\textbf{找鞍点}，$x$方向上找最小值，本来$\vv$方向上要找最大值，但容易到正无穷。因此换种方法$\iter{\vv}{k}$做一个保守的计算，每一步都走一个很小的步长。
